...
ToNumber[x_]:=Read[StringToStream[x],Number]
...
If[ATTDefaultValue[att] =="$$NoDefaultValue",
Print["$$$ :",attname,"  throw False 1!"];
Throw[False],
IAConcluded[insatt]=True;
IAConcludedValue[insatt]=ATTDefaultValue[att];
IAConcludedToDefaultValue[insatt]=True;
If[TraceReasoning==True,
	Print["*** ",attname,"[",InstanceNameFromInstance[instance],
		"]  concluded to defaultValue = ",ATTDefaultValue[att]]]
],
...
pr=BRPremisse[rule] /. CurrentInstance->instance;
	Catch[If[(aa=Evaluate[pr[[1]]];If[aa,True,False,Print["Premisse not evaluable: Rule : ",BRName[rule]," trying to evaluate: ",aa]];aa)==True, (* this catches the premisses fails *)
...
ExistingObject[ patternname_,attname_]:=Module[{AppliedPatternLst,InstanceResultLst,Instancelst,CLsymb,PTsymb=LookUpPattern[patternname],attvalue},
CLsymb=PatternClass[PTsymb];
AppliedPatternLst=Map[{#,attvalue=AttributeValue[#,attname];TestPattern[attvalue,PatternValue[PTsymb]]}&,CLInstanceList[CLsymb]];
InstanceResultLst=Select[AppliedPatternLst,(#[[2]]==True)&];
Instancelst=Map[(#[[1]])&,InstanceResultLst];
ConcludeClassAttributeValue[CLName[CLsymb],"instanceList:"<>patternname,Instancelst];
If[Length[Instancelst]>0,True,False]
]


ExistingObjectValue[ classname_,attname_, value_,localname_]:=Module[{AppliedPatternLst,InstanceResultLst,Instancelst,CLsymb,attvalue},
CLsymb=LookUpClass[classname];
AppliedPatternLst=Map[{#,attvalue=AttributeValue[#,attname];TestPattern[attvalue,value]}&,CLInstanceList[CLsymb]];
InstanceResultLst=Select[AppliedPatternLst,(#[[2]]==True)&];
Instancelst=Map[(#[[1]])&,InstanceResultLst];
IfNotExistingCreateClassAttribute[classname,"instanceList:"<>localname];
ConcludeClassAttributeValue[CLName[CLsymb],"instanceList:"<>localname,Instancelst];
If[Length[Instancelst]>0,True,False]
]


CheckExistingObjectValue[ classname_,attname_, value_,localname_]:=Module[{AppliedPatternLst,InstanceResultLst,Instancelst,CLsymb,attvalue},
CLsymb=LookUpClass[classname];
AppliedPatternLst=Map[{#,attvalue=AttributeValue[#,attname];TestPattern[attvalue,value]}&,CLInstanceList[CLsymb]];
InstanceResultLst=Select[AppliedPatternLst,(#[[2]]==True)&];
Instancelst=Map[(#[[1]])&,InstanceResultLst];
...
IfNotExistingCreateClassAttribute[classname_,attname_]:=
Module[{CLsymb,lst1,lst2},
CLsymb=LookUpClass[classname];
lst1=CLClassAttributeList[CLsymb];
lst2=Select[Range[AttributeNb],(ATTName[#]==attname)&];
If[Length[Intersection[lst1,lst2]]==0,
AddClassAttribute[classname,attname]]]


ListExistingobject[patternname_]:=Module[{patSymb,CLsymb,classname,attname},
patSymb=LookUpPattern[patternname];
CLsymb=PatternClass[patSymb];
classname=CLName[CLsymb];
attname="instanceList:"<>patternname;
ClassAttributeValue[classname,attname]]


ListExistingobjectValue[classname_,localname_]:=Module[{patSymb,CLsymb,attname},
attname="instanceList:"<>localname;
ClassAttributeValue[classname,attname]]


TestPattern[pattern1_,pattern2_]:=
Module[{},
If[Length[Position[pattern1,pattern2]]>0,True,False]
]


Contains[expr_,subexpr_]:=TestPattern[expr,subexpr]
...
If[TraceReasoning==True,Print["+++++concluding",attname,"[",InstanceNameFromInstance,"]"]];
	...
	If[TraceReasoning==True,Print["*** ",ATTName[IAAttribute[insatt]],"[",InstanceNameFromInstance[IAInstance[insatt]],"]","  concluded directly = ",value]];
	]
...
SetInitialValue[attname_,instance_,value_]:=Module[{att=LookUpAttribute[attname],insatt},
If[TraceReasoning==True,Print["+++++concluding",attname,"[",InstanceNameFromInstance,"]"]];
	insatt=InstanceAttributeValue[instance,att];
	If[NumberQ[insatt]==False,
		insatt=AddInstanciedAttribute[INSClass[instance],instance,att]]; (* pas forcement necessaire *)
	...
	If[TraceReasoning==True,Print["*** ",ATTName[IAAttribute[insatt]],"[",InstanceNameFromInstance[IAInstance[insatt]],"]","  concluded initially = ",value]];
	]


MemberMatchQ[elem_,Lst_]:=
Apply[Or,Map[(MatchQ[elem,#]==True)&,Lst]
]
